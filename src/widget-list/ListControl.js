...
export default class ListControl extends React.Component {
  ...
  constructor(props) {
    ...
    const itemsCollapsed =
      (value && Array(value.size).fill(listCollapsed)) || [];
    const keys =
      (value && Array.from({ length: value.size }, () => uuid())) || [];
  ...
  mixedDefault = (typeKey, type) => {
    const selectedType = this.props.field
      .get(TYPES_KEY)
      .find((f) => f.get("name") === type);
  ...
  addItem = (parsedValue) => {
    ...
    this.setState({
      ...
      keys: addToTop
        ? [itemKey, ...this.state.keys]
        : [...this.state.keys, itemKey],
    });
  ...
  handleChangeFor(index) {
    return (f, newValue, newMetadata) => {
      ...
      const listFieldObjectWidget =
        field.getIn(["field", "widget"]) === "object";
      const withNameKey =
        this.getValueType() !== valueTypes.SINGLE ||
    ...
    switch (valueType) {
      case valueTypes.MIXED: {
        ...
        const labelReturn = summary
          ? handleSummary(summary, entry, label, item)
          : label;
        return labelReturn;
      }
      case valueTypes.SINGLE: {
        ...
        const labelReturn = summary
          ? handleSummary(summary, entry, label, data)
          : label;
        return labelReturn;
      }
      case valueTypes.MULTIPLE: {
        ...
        const summary = field.get("summary");
        const labelReturn = summary
          ? handleSummary(summary, entry, value, item)
          : value;
  ...
  onSortEnd = ({ oldIndex, newIndex }) => {
    ...
    clearFieldErrors();
    this.validations = this.validations.filter((item) =>
      updatedKeys.includes(item.key)
    );
  };
  ...
  hasError = (index) => {
    ...
    if (fieldsErrors && fieldsErrors.size > 0) {
      return Object.values(fieldsErrors.toJS()).some((arr) =>
        arr.some(
          (err) =>
            err.parentIds && err.parentIds.includes(this.state.keys[index])
        )
      );
    }
  };
  ...
  renderItem = (item, index) => {
    ...
    return (
      <SortableListItem
        css={[
          styles.listControlItem,
          collapsed && styles.listControlItemCollapsed,
        ]}
        index={index}
        key={key}
      >
        ...
        <TopBarButton onClick={partial(this.handleRemove, index)}>
  ...
  renderListControl() {
    ...
    const labelSingular =
      field.get("label_singular") || field.get("label", field.get("name"));
    const listLabel =
      items.size === 1 ? labelSingular.toLowerCase() : label.toLowerCase();
    const minimizeCollapsedItems = field.get("minimize_collapsed", false);
    const allItemsCollapsed = itemsCollapsed.every((val) => val === true);
    const selfCollapsed =
      allItemsCollapsed && (listCollapsed || !minimizeCollapsedItems);
        ...
        {({ cx, css }) => (
          ...
          >
            <ObjectWidgetTopBar
              ...
              onAddType={(type) =>
                this.handleAddType(type, resolveFieldKeyType(field))
              }
